...
const ENCRYPTION_KEY =
  process.env.ENCRYPTION_KEY || 'ahbaofa4hae7IeBoh1PhaaXeitieV7';
...
function testDecryption() {
  const testToken = process.argv.find(
    (arg, i) => process.argv[i - 1] === '--test'
  );

  if (!testToken) {
    console.log(
      'Usage: node decrypt-tokens.js --test "encrypted_token_string"'
    );
    console.log(
      'Example: node decrypt-tokens.js --test "k+n8vospSptBydGXEon4MMCTefXYWiR1yzGh+TJWZRxm1Lr+RKHpeJ4Me/oHTkja"'
    );
    return;
  }
  ...
  try {
    ...
    if (
      decrypted.startsWith('ghp_') ||
      decrypted.startsWith('gho_') ||
      decrypted.length >= 40
    ) {
    ...
    console.log(
      '3. Ensure the encryption algorithm matches SQLAlchemy-Utils settings'
    );
  }
}
